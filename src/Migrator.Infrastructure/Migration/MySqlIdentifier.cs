...
internal static partial class MySqlIdentifier
{
    ...
    public static string Backtick(string identifier)
    {
        ...
        switch (parts.Length)
        {
            case 1:
                throw new InvalidOperationException(
                    $"Nombre de tabla MySQL no válido: '{qualifiedName}'. Use 'esquema.tabla' (p. ej. el valor devuelto al cargar tablas).");
            case 2:
                schema = parts[0];
                table = parts[1];
                break;
            default:
                throw new InvalidOperationException(
                    $"Nombre de tabla MySQL no válido: '{qualifiedName}'. Use 'esquema.tabla'.");
        }
    ...
    private static void ValidatePart(string part)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Identificador MySQL no permitido: '{part}'. Use solo letras, números y guion bajo; convención esquema.tabla.");
        }
    }
}
